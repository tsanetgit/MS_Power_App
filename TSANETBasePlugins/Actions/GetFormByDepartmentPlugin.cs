...
public class GetFormByDepartmentPlugin : IPlugin
{
    public void Execute(IServiceProvider serviceProvider)
    {
        ...
        try
        {
            ...
            int departmentId = (int)context.InputParameters["DepartmentId"];
            if (departmentId <= 0)
            {
                throw new InvalidPluginExecutionException("Valid Department ID is required.");
            }
            ...
            ApiResponse response = commonIntegration.GetFormByDepartment(departmentId, accessToken).Result;
